...
class RASUSampler:
    def __init__(self, features, target):
        """
        风险感知分层欠采样器（简化版，不进行特征选择）
        """
        self.data = pd.concat([features, target], axis=1)
        self.target_name = target.name
        self.risk_features = features.columns.tolist()  # 使用所有特征
    ...
    def precompute_risk_weights(self):
        """预计算特征风险权重（简化版）"""
        print("\nPrecomputing risk weights...")

        # 直接使用所有特征，不计算特征重要性
        print(f"Using all {len(self.risk_features)} features")

        # 计算每个特征的风险权重（基于分箱的缺陷率）
        self.risk_weights = {}
        for feature in self.risk_features:
            try:
                ...
                if n_unique <= 1:
                    ...
                    self.risk_weights[feature] = {None: 1.0}  # 默认权重
                    continue
                ...
                print(f"  Feature '{feature}' binning:")
    ...
    def adaptive_sampling(self):
        ...
        for feature in self.risk_features:
            ...
            if bin_col not in self.data.columns or feature not in self.risk_weights:
                continue
            ...
            try:
                ...
                # 处理缺失值
                if weights.isna().any():
                    weights = weights.fillna(0.5)  # 使用中性权重
...
def light_feature_selection(X, feature_importance, threshold=0.01):
    """
    轻度特征筛选：移除重要性低于阈值的特征
...
def rasu_only_evaluation(file_path, target_column='bug', n_folds=5):
    ...
    y = data[target_column].apply(lambda x: 1 if x > 0 else 0)
    ...
    # 标准化
    scaler = RobustScaler()
    X_preprocessed = pd.DataFrame(scaler.fit_transform(X_imputed), columns=X_imputed.columns)

    # 3. 交叉验证
    skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=42)
    all_metrics = {'Precision': [], 'Recall': [], 'F1 Score': [], 'MCC': [], 'AUC': []}
    sampling_ratios = []
...
if __name__ == "__main__":
    # 从CSV文件加载真实数据
    data_path = r"D:\-\code\datacunshu\ant-1.7.csv"  # 请替换为你的实际路径
