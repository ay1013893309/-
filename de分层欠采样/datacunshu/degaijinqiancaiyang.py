...
class DEFeatureSelector:
    ...
    def enhanced_objective(self, feature_mask):
        ...
        try:
            ...
            # 使用分层交叉验证
            cv = StratifiedKFold(n_splits=min(3, len(np.unique(self.y))),
...
class RASUSampler:
    ...
    def precompute_risk_weights(self):
        ...
        baseline_score = np.mean(cross_val_score(model, X, y, cv=cv, scoring='roc_auc'))

        for feature in self.risk_features:
            X_reduced = X.drop(feature, axis=1)
            reduced_score = np.mean(cross_val_score(model, X_reduced, y, cv=cv, scoring='roc_auc'))
...
if __name__ == "__main__":
    # 从CSV文件加载真实数据
    data_path = r"G:\pycharm\lutrs\de分层欠采样\datacunshu\ant-1.6.csv"  # 使用原始字符串
