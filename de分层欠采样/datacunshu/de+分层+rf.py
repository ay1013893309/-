...
class DEFeatureSelector:
    ...
    def enhanced_objective(self, feature_mask):
        ...
        # 在函数顶部计算召回率（新增）
        try:
            recall = recall_score(self.y, model.predict(X_sub))
        except:
            recall = 0.4  # 默认值
        # # 最终适应度 (最大化问题)
...
class RASUSampler:
    ...
    def precompute_risk_weights(self):
        ...
        for feature in self.risk_features:
            ...
            reduced_score = np.mean(cross_val_score(model, X_reduced, y, cv=cv, scoring='recall'))
...
if __name__ == "__main__":
    # 从CSV文件加载真实数据
    data_path = r"G:\pycharm\lutrs\stability-of-smote-main\AEEM\converted_Lucene.csv"  # 使用原始字符串
