...
from imblearn.metrics import geometric_mean_score  # 添加gmean计算
import csv
import warnings
...
# 初始化结果文件 - 添加三个新指标
metrics = ['auc', 'balance', 'recall', 'pf', 'brier', 'mcc', 'precision', 'f1', 'gmean']
writers = {}
stable_writers = {}
...
for inputfile in os.listdir(nasa_dir):
    ...
    try:
        ...
        for j in range(10):
            ...
            results['f1'].append(f1_score(test_y, predict_result))
            results['gmean'].append(geometric_mean_score(test_y, predict_result))
            ...
            if stable_train_data is not None and len(stable_train_data) > 0:
                ...
                # 新增的三个指标
                stable_results['precision'].append(tp / (tp + fp) if (tp + fp) > 0 else 0)
                stable_results['f1'].append(f1_score(test_y, stable_predict_result))
                stable_results['gmean'].append(geometric_mean_score(test_y, stable_predict_result))
